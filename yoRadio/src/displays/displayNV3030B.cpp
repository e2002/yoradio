...
#if DSP_HSPI
DspCore::DspCore(): Adafruit_NV3030B(&SPI2, TFT_CS, TFT_DC, TFT_RST) {}
#else
DspCore::DspCore(): Adafruit_NV3030B(TFT_CS, TFT_DC, TFT_RST) {}
#endif
...
void DspCore::startWrite(void) {
  TAKE_MUTEX();
  Adafruit_NV3030B::startWrite();
}

void DspCore::endWrite(void) {
  Adafruit_NV3030B::endWrite();
  GIVE_MUTEX();
}
...
void DspCore::writePixel(int16_t x, int16_t y, uint16_t color) {
  ...
  Adafruit_NV3030B::writePixel(x, y, color);
}
...
void DspCore::writeFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
  ...
  Adafruit_NV3030B::writeFillRect(x, y, w, h, color);
}
